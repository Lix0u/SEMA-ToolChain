...
class RadareFunctionAnalyzer:
        ...
        while s < end_address:
            ...
            if self.arch == "x86":
                filtered_instruction = (
                    "X_" + RadareFunctionAnalyzer.filter_memory_references(instruction)
                )
            elif self.arch == "arm":
                filtered_instruction = (
                    "A_" + RadareFunctionAnalyzer.filter_memory_references(instruction)
                )
            ...
            if s in calls and depth > 0:
                if calls[s] in functions_dict:
                    ii, aa = self.function_to_inst(
                        functions_dict, functions_dict[calls[s]], depth - 1
                    )
    ...
    def analyze(self):
        ...
        for my_function in function_list:
            ...
            try:
                instructions, asm = self.function_to_inst(
                    functions_dict, my_function, self.top_depth
                )
    ...
    def find_functions_boudaries(self):
        if self.use_symbol:
            function_list = self.find_functions_by_symbols()
        else:
            function_list = self.find_functions()
        ...
        for my_function in function_list:
            function_boundaries.append(
                {
                    "minbound": my_function["minbound"],
                    "maxbound": my_function["maxbound"],
                }
            )
